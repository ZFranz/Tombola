...
public class Cartella {
	...
	final private boolean[] segnati; // true se il numero corrispondente è
										// segnato
	...
	public Vincita vincitaMax() {
		...
		// Niente tombola, calcola la riga migliore (quella con più numeri
		// segnati)
	...
	public boolean numeroEstratto(final int numero) {
		// cerca il numero tra quelli presenti nella cartella, e se presente lo
		// segna estratto
		final int indice = Utility.indexOf(numero, numeri, 15);
		if (indice >= 0) {
			segnati[indice] = true;
			return true;
		}
	...
	public void stampa() {
		for (int r = 0; r < 3; r++) {
			...
			for (int c = 0; c < 5; c++) {
				...
				// stampa numero
				output += String.format("%02d%c", num, (segnati[index] ? '#' : ' '));
			}
